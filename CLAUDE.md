...
- **One `entries` table, not per-domain tables.** `type` (`task | log | event`) + `category` (chip-picked, see `CATEGORIES` in `db/types.ts`) together classify everything. "note" used to be a fourth type but was merged into "log" (`migrations/0003_merge_notes_into_logs.sql`) since they were never meaningfully distinct. Adding a new category is a UI constant change, not a migration.
...
- **Chat capture (`/capture`) is the only entry point.** The old chip-based `/capture/manual` fallback has been removed. Typed messages are parsed into `entries` rows by an LLM via OpenRouter (`src/lib/ai/parse-entry.ts`), auto-saved immediately. Uncertain parses get `needs_review = true` instead of blocking on a confirm step: that flag surfaces as a badge on `EntryCard` wherever the entry is shown. `OPENROUTER_API_KEY` is server-only, read in `src/lib/ai/openrouter.ts`, never `NEXT_PUBLIC_`.
- **One message can log multiple things.** `parseMessage()` first classifies intent (`"new"` vs `"edit"`), and for `"new"` returns an array of 1-to-N entry drafts, not a single object — "pay rent and call mom tomorrow" becomes two tasks in one bulk insert (`src/app/capture/chat-actions.ts`). It only splits on genuinely distinct things; a single sentence with extra clauses stays one entry (verified live against the real model, not just unit tests, since this is a judgment call the model has to make).
- **Editing from chat is a two-call, DB-search-first pipeline, and it never auto-applies.** When `parseMessage()` classifies intent as `"edit"`, it also returns `editQuery` (a few keywords). `searchEntriesForEdit()` (`src/lib/db/queries.ts`) does a deterministic `ilike` search over the user's own entries to get candidates — the LLM never free-associates across the whole dataset. Only then does `resolveEdit()` (second call) see those candidates and decide which one matches and what changed. Regardless of outcome, `sendCaptureMessage` (`src/app/capture/chat-actions.ts`) does *not* write to `entries`: it returns a `PendingEdit` (either `{ kind: "edit", entryId, fields }` or `{ kind: "new-fallback", fields }` when nothing matched or the model wasn't confident), which `ChatCapture` renders as `PendingEditCard` — a prefilled, still-editable form with confirm/cancel, reusing `EntryEditForm`. Nothing is ever silently dropped *or* silently wrong: a message with edit intent always ends in an explicit user confirm, never an auto-apply. This replaced the old mechanism (a 15-minute "correction window" that auto-applied and could only ever fix the single most-recently-created entry) entirely. The "restart chat" button in `ChatCapture` still exists, but now only bounds *context* fed to the "new entry" classification (so old unrelated chat history doesn't bleed into a new entry's title), unrelated to edit confirmation.
- **Entries are editable in place**, not just create/toggle-done/delete. `EntryCard` has an inline edit mode (pencil button) using the shared `EntryEditForm` (type/category/title/body/due date/amount), saved via `updateEntry()` in `src/app/entries/actions.ts`. The same form component is reused by `PendingEditCard` in chat (see above), so both surfaces edit identically.
- **`capture_chat` holds the chat transcript**, not the entries themselves. One constant, `CHAT_SESSION_WINDOW_MS` (`src/lib/db/queries.ts`, currently 15 minutes), defines "still the same conversation" for two things at once: how far back `/capture` looks when loading the visible thread, and how much chat history the parser gets as context. Kept short and shared on purpose — coming back after a gap to log something unrelated shouldn't dump you into a stale old thread, and without a matching cutoff on the *context* side the LLM has no sense that time passed and will blend a genuinely new, unrelated message into a new entry's title. Only the last ~6 rows are sent as context per request regardless, to keep token cost bounded.
...
- **Editing entries:** `EntryCard` supports inline edit (pencil button) covering all fields, plus toggle-done and delete. No reordering, and no bulk edit UI.
